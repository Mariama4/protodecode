...
from .utils import buffer_to_pretty_hex, parse_input_data
from .varint import decode_varint_parts

MAX_REASONABLE_FIELD_INDEX = 1_000_000
...
def decode_proto(buffer: bytes):
    ...
    return {"parts": parts, "leftOver": reader.read_buffer(reader.left_bytes())}
...
def is_reasonable_nested_protobuf(decoded):
    if len(decoded["leftOver"]) != 0 or not decoded["parts"]:
        return False
    for part in decoded["parts"]:
        if part["index"] <= 0 or part["index"] > MAX_REASONABLE_FIELD_INDEX:
            return False
    return True
...
def get_protobuf_part(part):
    if part["type"] == TYPES["VARINT"]:
        ...
        if is_reasonable_nested_protobuf(decoded):
